...
<div align="center"><span class="стиль24">Затвердження</span></div>
<?php
if (!($dostup&8)) echo "<META HTTP-EQUIV=\"Refresh\" content =\"0; URL='index.php'\">"; 
else
{
  $date_now = time();										  
  $result=pg_query($connection, "select * from auto a, user_auto u where a.podav = u.user_auto  and a.data_prib > $date_now and a.stan <> 'Створено' and a.stan <> 'Не потребує погодження' order by a.stan desc, a.data_otpr;");	
  ?>
  ...
  <?php 
	  ...
	  if(($db['stan'] == "Затверджено, пог. з автобазою")|($db['stan'] == "Затверджено")) echo " bgcolor=\"##99FF99\""; else 	
	   echo " bgcolor=\"#FFFFFF\"";
  ...
  ?>  	
	  ...
	  onclick="event.cancelBubble=true;this.select();lcs(this)" value="<?php echo date( 'd\-m\-Y', $db['data_prib']); ?>" size="8" />
		...
		<td><?php echo $db['stan'];$db['stan'] ?>&nbsp;
		  <input name="id" type="hidden" id="id" value="<?php echo $db['auto_id']; ?>">
		 <?php 
		 if(($db['stan'] == "Затверджено, пог. з автобазою")|($db['stan'] == "Затверджено"));
		 else {
		 ?>
		  <br><input type="submit" name="Submit1" value="Затвердити" >
		 <?php
		  };
