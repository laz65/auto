...
$result2=pg_query($connection, "select dostup from user_auto where login = '$login';");
$db2=pg_fetch_array($result2);
$dostup=$db2['dostup'] ;
$date = `sudo -i date +%a_%b_%d_%Y_%X_GMT%:::z`;

echo " Мой пароль $login с доступом $dostup !!! Дата = $date";
	$fp = fopen ("files_probe.txt", "w");
	fwrite($fp,"Proverka");
	fclose($fp);
