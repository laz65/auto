...
if(isset($_POST['Submit'])) 
	{
		...
		$date_nn = (integer)$_POST['date_nn'];
		$date_kk = (integer)$_POST['date_kk'];
		...
		if (pg_query($connection, "UPDATE auto set stan = '$stan', history = '$history', data_otpr = $date_nn, data_prib = $date_kk where auto_id = $auto_id ;")) 
			{
				if ($stan == 'Відмовлено')
				{ 
					if($dostup>>3&1) echo "<strong>В заявці відмовлено!</strong><br>","<META HTTP-EQUIV=\"Refresh\" content =\"2; URL='zatv.php'\">"; 
					else echo "<strong>В заявці відмовлено!</strong><br>","<META HTTP-EQUIV=\"Refresh\" content =\"2; URL='zatv0.php'\">";
				}
				else
				{
					if($dostup>>3&1) echo "<strong>Стан заявки змінено на $stan!</strong><br>","<META HTTP-EQUIV=\"Refresh\" content =\"2; URL='zatv.php?id=",$auto_id,"'\">"; 
					else echo "<strong>Стан заявки змінено на $stan! </strong><br>","<META HTTP-EQUIV=\"Refresh\" content =\"2; URL='zatv0.php'\">";
				}
...
if(isset($_GET['id']))
	{
		$auto_id = $_GET['id'];		
		$result=pg_query($connection, "select data_otpr, data_prib, mobil, stan from auto where auto_id = $auto_id;");	
		$db=pg_fetch_array($result);
		if(isset($_GET['Submit2'])) $stan = "Відмовлено" ;
		if(isset($_GET['Submit'])) $stan = "Погоджено" ;
		if(isset($_GET['Submit1'])) 
		{
			if ($db['stan'] == "Погоджено" | $db['stan'] == "Відмовлено") $stan = "Затверджено" ;
			if  ($db['stan'] == "Погоджено, пог. з автобазою") $stan = "Затверджено, пог. з автобазою";
		}
...
if(pg_num_rows($result) > 0) 
	{
		if(pg_num_rows($result) > 1) echo "Для цього автомобіля, можливо, існує декілька заявок на один час(темні - інші):";
?>                                              
...
?>	
	...
	if($db['auto_id'] != "$auto_id") echo " bgcolor=\"#888888\""; 
		...
		{
			if($db['stan'] == "Погоджено") echo " bgcolor=\"#99FF99\""; 
	...
	>
	  ...
	  <td><?php if($db['auto_id'] != "$auto_id") echo date( 'H\:i d\-m\-Y', $db['data_otpr']); else echo date( 'H\:i d\-m\-Y', $date_nn);?>&nbsp;</td>
	  <td><?php if($db['auto_id'] != "$auto_id") echo date( 'H\:i d\-m\-Y', $db['data_prib']); else echo date( 'H\:i d\-m\-Y', $date_kk); ?>&nbsp;</td>
		...
		if ($stan == 'Погоджено')
			{
				...
				include('Mail.php');
				 ...
				 $subject = "Погоджена заявка на автомобіль";
...
//_________________________________________________________________________________				
				 ...
				 // получение логинов работников автобазы и админов
				    $result33=pg_query($connection, "select login, dostup from  user_auto  where dostup & 6 <> 0 and pass <> '';");	
					$db33=pg_fetch_array($result33);
					$to = $db33['login']."@ukrtelecom.ua";	
...
//____________________________________________________________________________________________
				 $from = "zayavky@ukrtelecom.ua"; 
				 // получение логинов Затверджуючих
				    $result33=pg_query($connection, "select login, dostup from  user_auto  where dostup & 8 <> 0 and pass <> '';");	
					$db33=pg_fetch_array($result33);
					$to = $db33['login']."@ukrtelecom.ua";	
					while ($db33=pg_fetch_array($result33)):
					$to = $to.",".$db33['login']."@ukrtelecom.ua";				
			   ...
			   "\nМашина		- ".$db['mobil'].		 
			   "\nДата та час виїзду	- ".date( 'H\:i d\-m\-Y', $db['data_otpr']).
			 "\n\nЗатвердження: http://10.80.11.106/auto/zatv.php "; 		 
...
{
?>	
	<input type="hidden"  name="zatv" value="<?php echo $user_auto; ?>">
<?php	
}
else
{
	if($stan == "Погоджено") echo "Хто погодив заявку:"; else 
	echo "Хто відмовив в заявці:";
?>
  
  <select name="zatv" id="zatv">
  <?php

$result2=pg_query($connection, "select * from user_auto where dostup & 1 <> 0;");
