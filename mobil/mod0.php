...
<?php
...
else
	{
		...
		if(pg_num_rows($result) > 0) 
		 ...
		 $ip = trim($_POST["NAC"]);
		 $dostup = $_POST["DOSTUP"];
			if (pg_query($connection, "update user_auto set telefone = '$telefone',  pidr = '$pidr', posada = '$posada' where login = '$login';")) 
				{ 
				echo "<strong>Запис створений, чекайте відповідь з логіном на паролем по електронній пошті!</strong><br><br><a href = 'index.php'><strong>Подивитись активні заявки на автотранспорт.</strong></a>";
				include('Mail.php');
				$from = "zayavky@ukrtelecom.ua";
				$to = "";
				$result3=pg_query($connection, "select login from  user_auto  where dostup = 3 and pass <> '';");	
